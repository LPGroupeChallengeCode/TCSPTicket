...
module.exports = function(app) {
	...
	app.post('/login', function(req, res, next){
		...
		User.findOne({mail: req.body.mail, password: md5(req.body.password)}, function(err, user){
				if(err){console.log(err); return next(err);}
				if(!user)
				{
					res.writeHead(302, { 
	...
	app.post('/user/password', function(req, res){
		User.findOneAndUpdate({ email: req.body.email }, { password: md5(req.body.newPassword) }, function(err, user){
			res.json("le mot de passe a été modifié");
		});
	});
}