...
module.exports = function(app) {
	...
	app.post('/paypal/paynow', function(req, res){
		...
		paypal.payement.create(payement, function(error, payement){
			if(error)
			{
				console.log(error);
			}
			...
			{
				if(payement.payer.payement_method === 'paypal')
				{
					req.payementId = payement.id;
					var redirectUrl;

					for(var i=0; i < payement.links.length; i++)
					{
						var link = payement.links[i];
